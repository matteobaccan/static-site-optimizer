...
const { getImageDimensions } = require('../assets/dimensions');
...
function fixImgTags(html, resolveImagePath) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if ((!('width' in attrs) || !('height' in attrs)) && tag.attrs.src) {
      ...
      if (absPath && fs.existsSync(absPath)) {
        ...
        if (dims) {
          ...
          changed = true;
          findings.push({ code: 'missing-img-dimensions', category: 'performance', autoFixed: true, message: `width/height added to ${tag.attrs.src}` });
        }
      }
    }
    ...
    if (i >= 2 && !('loading' in attrs)) {
      attrs.loading = 'lazy';
      changed = true;
      findings.push({ code: 'missing-lazy-loading', category: 'performance', autoFixed: true, message: `loading="lazy" added to ${tag.attrs.src || '(image with no src)'}` });
    }
