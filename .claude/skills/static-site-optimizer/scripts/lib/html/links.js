...
function fixExternalLinks(html) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    findings.push({ code: 'missing-noopener', category: 'performance', autoFixed: true, message: 'rel="noopener noreferrer" added to a target="_blank" link' });
  }
      ...
      if (inAnchor) {
        ...
        if (token.content.includes('++39')) {
          ...
          findings.push({
            ...
            category: 'seo',
            autoFixed: true,
            message: `Replaced '++39' with '+39' in the existing link text`
          });
        }
        continue;
      }
        ...
        for (const m of nonOverlapping) {
          ...
          if (m.type === 'email') {
            ...
            findings.push({
              ...
              category: 'seo',
              autoFixed: true,
              message: `Email ${m.value} made clickable with mailto:`
            });
            ...
            findings.push({
              ...
              category: 'seo',
              autoFixed: true,
              message: `Phone number ${m.value} made clickable with tel:`
            });
          }
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (href && href.includes('@') && !href.toLowerCase().startsWith('mailto:') && !href.toLowerCase().startsWith('http')) {
      ...
      findings.push({
        ...
        category: 'seo',
        autoFixed: true,
        message: `Added the missing mailto: to an anchor href`
      });
    }
    ...
    if (href && /^\+?\+?\d+$/.test(phoneCleanTest) && phoneCleanTest.length >= 7 && !href.toLowerCase().startsWith('tel:') && !href.toLowerCase().startsWith('http')) {
      ...
      findings.push({
        ...
        category: 'seo',
        autoFixed: true,
        message: `Added the missing tel: to an anchor href`
      });
    }
  ...
  for (const tag of tags) {
    ...
    if (/\.(docx?|pptx?)$/i.test(cleanHref)) {
      findings.push({
        ...
        autoFixed: false,
        message: `Link to an Office document (${href}). Converting it to PDF is recommended for accessibility and compatibility.`
      });
    }
  }
